...
const BookCard = ({ data, favourite }) => {
  ...
  const handleRemoveBook = async () => {
    const response = await axios.put(
      "https://bookstore-with-admin.onrender.com/api/v1/remove-book-from-favourite",
      {},
      { headers }
    );
  ...
  return (
    <div className='bg-zinc-800 min-h-[39vh] md:min-h-[50vh] rounded p-4 flex-col hover:bg-zinc-700'>
      <Link to={`/view-book-details/${data._id}`}>
        <div className=''>
          <div className='bg-zinc-900 rounded flex items-center justify-center'>
            <img src={data.url} alt="/" className='h-[15vh] md:h-[25vh]' />
          </div>
