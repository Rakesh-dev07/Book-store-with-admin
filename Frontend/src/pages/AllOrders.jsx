...
const AllOrders = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      const response = await axios.get(
        "https://bookstore-with-admin.onrender.com/api/v1/get-all-orders",
        { headers }
      );
  ...
  const submitChanges = async (i) => {
    ...
    const response = await axios.put(
      `https://bookstore-with-admin.onrender.com/api/v1/update-status/${id}`,
      Values,
      { headers }
    );
  ...
  return (
    <>
      {!AllOrders && (
        <div className='h-[100%] bg-zinc-900 flex items-center justify-center'>
          <Loader />{" "}
        </div>
      )}
              ...
              <div className='bg-zinc-800 w-full rounded py-2 px-4 flex gap-2 hover:bg-zinc-900 hover:cursor-pointer transition-all duration-300'>
                ...
                <div className='w-[30%] md:w-[16%]'>
                  <h1 className='font-semibold'>
                    ...
                    <div
                      className={`${Options === i ? "flex" : "hidden"
                        } flex mt-4`}
                    >
                      <select
                        name="status"
